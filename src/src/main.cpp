...
// ====== Berry VM (勿修改) ======
bbe_vm* berry_vm = nullptr;

// ---------- C → Berry 绑定 ----------
static int b_led_on(bbe_vm* vm)      {{ digitalWrite(LED_PIN, HIGH); be_return_nil(vm); }}
static int b_led_off(bbe_vm* vm)     {{ digitalWrite(LED_PIN, LOW);  be_return_nil(vm); }}
static int b_led_toggle(bbe_vm* vm)  {{ digitalWrite(LED_PIN, !digitalRead(LED_PIN)); be_return_nil(vm); }}

static int b_gpio_set(bbe_vm* vm) {{
    pinMode(be_toint(vm,1), OUTPUT);
    digitalWrite(be_toint(vm,1), be_toint(vm,2)?HIGH:LOW);
    be_return_nil(vm);
}}
static int b_gpio_read(bbe_vm* vm)   {{ be_pushint(vm, digitalRead(be_toint(vm,1))); be_return(vm,1); }}
static int b_delay_ms(bbe_vm* vm)    {{ delay(be_toint(vm,1)); be_return_nil(vm); }}

static int b_pwm_duty(bbe_vm* vm) {{
...
static int b_pwm_freq(bbe_vm* vm) {{
    ledcSetup(0,be_toint(vm,2),10); ledcAttachPin(be_toint(vm,1),0);
    be_return_nil(vm);
}}

static int b_analog_read(bbe_vm* vm) {{ be_pushint(vm, analogRead(be_toint(vm,1))); be_return(vm,1); }}

// DHT11/DHT22 — 返回 Berry map: {"temp": 25.0, "humidity": 60.0}
static int b_dht_read(bbe_vm* vm) {{
...
// UART
static int b_uart_write(bbe_vm* vm) {{
...
// SPI
static int b_spi_xfer(bbe_vm* vm) {{
    SPI.beginTransaction(SPISettings(1000000,MSBFIRST,SPI_MODE0));
    uint8_t b=SPI.transfer(be_toint(vm,1));
    SPI.endTransaction();
    be_pushint(vm,b); be_return(vm,1);
}}

// MQTT 发布 + 日志
WiFiClient wific; PubSubClient mqtt(wific);
static int b_mqtt_pub(bbe_vm* vm) {{
    mqtt.publish(be_tostring(vm,1), be_tostring(vm,2));
    be_return_nil(vm);
}}
static int b_log(bbe_vm* vm)        {{ Serial.println(be_tostring(vm,1)); be_return_nil(vm); }}
static int b_device_id(bbe_vm* vm)  {{ be_pushstring(vm,DEVICE_ID); be_return(vm,1); }}
static int b_free_heap(bbe_vm* vm)  {{ be_pushint(vm,ESP.getFreeHeap()); be_return(vm,1); }}
static int b_millis_(bbe_vm* vm)    {{ be_pushint(vm,millis()); be_return(vm,1); }}
